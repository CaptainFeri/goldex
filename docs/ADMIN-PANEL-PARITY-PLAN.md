...
A frontend developer's first question is _what comes back_, and that is exactly
...
Swagger will document the _inner_ shape while the wire carries the envelope,
...
**Only in `goldex-admin-panel`** — and these are _working, API-wired_ pages, not
...
needs two admin panels at all, or whether `goldex-admin-panel` is really _the
reference implementation and staging ground_ while `ui-parszargar` is the
...
- **Arbitrage.** That subsystem works in toman end to end: the signals come from
  ...
  the platform's rial ledger, so converting it would _introduce_ the ten-fold
...
the list refetches every 3s _only while_ something is `pending` or `running`
...
shift a picked day _backwards_ for any timezone east of UTC — Tehran included —
...
kept below the new section. What _was_ removed is only what the new endpoints
