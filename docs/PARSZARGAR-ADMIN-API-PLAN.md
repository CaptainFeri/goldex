...
3. **Accounting vouchers.** `AccountingDocumentPage` is a double-entry voucher
   ...
   voucher book.
4. **EM withdrawals — a _view_ gap, not a capability gap.** Confirmed as the
...
5. **Unit presentation.** The backend is rial throughout; the panels show
   ...
   panel (§3.1).
6. **Service-provider registry.** `ProvidersPage` tracks 13 _infrastructure_
...
{
  "status": 422,
  "message": "مبلغ برداشت از سقف روزانه بیشتر است",
...
`is_ticker`, `display_order` and `category`. A _price_, though, belongs to a
...
The response gives `buyPrice`/`sellPrice` (the _display_ prices, carrying the
...
- **Printable documents** (EM receipt, provider settlement invoice, Shahin
  statement) — the UI _already_ owns pixel-perfect print CSS
...
the rule is _identity is frozen, configuration is not_:
...
```jsonc
{
  ...
  "purity": "750",
  "angNumber": "A-102", // material only
  "cryptoType": "USDT",
  "network": "TRC20",
  "textId": "TX-9876", // crypto only
  "fiatType": "دلار آمریکا",
  "sorter": "done",
} // fiat only
```
...
_Arbitrage is not a report type._ The panels' form offers **آربیتراژ**, but
...
_PDF is not a format._ §4.7 already settled that printable documents stay
...
guessing a UUID, so `/:id/download` re-checks ownership and returns 404 (not 403) to a non-owner, which avoids confirming that the id exists.
...
| _(none for EM)_                                        | §5.17 is a projection over the existing `p2p_*` tables; the only new column is `p2p_withdraw_request.has_enclosure` |
...
> > > > > > > c59821be9a834acd9c22ede014e8ad1f22950728
...
> > > > > > > | 5 | Fixed roles — editable? | _Our call:_ identity frozen, configuration editable, permissions editable except the root role | §5.7 table + 3 escalation invariants + `capabilities` on the payload |
