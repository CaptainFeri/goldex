...
function fundingBadge(o: any) {
  const v = String(
    o?.fundingSource ?? (o?.isCreditLinked ? "CREDIT" : "WALLET"),
  ).toUpperCase();
  if (v === "CREDIT") {
    return (
      <div>
...
function AdminEditModal({
  ...
  const [price, setPrice] = useState(
    String(toFormAmount(order?.price, quote) ?? ""),
  );
  ...
  return (
    <Modal title="ویرایش سفارش توسط ادمین" onClose={onClose}>
      <form onSubmit={submit}>
        ...
        <div
          className="row"
          style={{ justifyContent: "flex-end", gap: 10, marginTop: 16 }}
        >
...
function OrderDetailsModal({
  ...
  const q = useQuery({
    ...
    queryFn: async () =>
      unwrap<AdminOrder>((await api.get(`/admin/orders/${orderId}`)).data),
  });
  const order = q.data;
  return (
    <Modal
      title={`جزئیات سفارش ${order?.orderCode ?? orderId?.slice(0, 8)}`}
      onClose={onClose}
      wide
    >
      {q.isLoading ? (
        ...
        <>
          <div className="grid grid-3" style={{ marginBottom: 16 }}>
            ...
            <div className="field">
              <label>مقدار</label>
              <div className="mono">
                {fmtBySymbol(order.quantity, baseSlug(order.pricePair), {
                  digits: 4,
                })}
              </div>
            </div>
            ...
            <div className="field">
              <label>ارزش کل</label>
              <div className="mono">
                {fmtBySymbol(order.totalValue, quoteSlug(order.pricePair), {
                  digits: 0,
                })}
              </div>
            </div>
...
export default function OrdersPage() {
  ...
  const list = useQuery({
    ...
    queryFn: async () =>
      unwrap<any>((await api.get("/admin/orders", { params })).data),
  });

  const cancel = useMutation({
    mutationFn: (id: string) =>
      api.delete(`/admin/orders/${id}/cancel`, {
