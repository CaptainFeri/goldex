...
interface Summary {
  ...
  reference: { symbolId: string; name: string | null; slug: string | null; isDefault: boolean };
  valuationBasis: ValuationBasis;
  assets: AssetLine[];
...
function SettingsCard({ settings }: { settings: AccountingSettings }) {
  ...
  const [valuationBasis, setValuationBasis] = useState<ValuationBasis>(settings.valuationBasis);
  const [staleness, setStaleness] = useState(String(settings.priceStalenessSeconds));
  ...
  const save = useMutation({
    mutationFn: () =>
      api.patch("/admin/accounting/settings", {
        referenceSymbolId,
        valuationBasis,
        priceStalenessSeconds: Number(staleness) || settings.priceStalenessSeconds,
      }),
    onSuccess: () => {
  ...
  return (
    <Card title="تنظیمات حسابداری">
      <div className="muted" style={{ fontSize: 12, marginBottom: 12 }}>
        نماد قیمت‌دهی، ارز مرجع گزارش‌هاست: سود، هزینه و سود خالص هر دارایی با قیمت لحظه‌ای به این نماد
        تبدیل و جمع می‌شود. تا وقتی نمادی انتخاب نشده باشد، گزارش‌ها بر مبنای ریال محاسبه می‌شوند.
      </div>
      ...
      {settings.effectiveReference.isDefault && (
        <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>
          ...
          {settings.effectiveReference.slug ?? settings.effectiveReference.name} محاسبه می‌شوند.
        </div>
      )}
...
export default function AccountingPage() {
  ...
  const settings = useQuery({
    ...
    queryFn: async () => unwrap<AccountingSettings>((await api.get("/admin/accounting/settings")).data),
  });
  ...
  const summary = useQuery({
    ...
    queryFn: async () =>
      unwrap<Summary>(
        (await api.get("/admin/accounting/summary", { params: { from: range.from, to: range.to } })).data
      ),
  });

  const holdings = useQuery({
    queryKey: ["accounting-holdings"],
    queryFn: async () => unwrap<any>((await api.get("/admin/accounting/holdings")).data),
  });
  ...
  return (
    <>
      {settings.isLoading ? (
        <Card title="تنظیمات حسابداری"><Loading /></Card>
      ) : settings.isError ? (
        <Card title="تنظیمات حسابداری"><ErrorState message={apiError(settings.error)} /></Card>
      ) : settings.data ? (
        ...
        {summary.isLoading ? (
          ...
          <>
            <div className="grid grid-3" style={{ marginBottom: 16 }}>
              <Stat label={`سود ناخالص (${unit})`} value={fmtNum(data.totals.revenue, 0)} />
              <Stat label={`هزینه (${unit})`} value={fmtNum(data.totals.cost, 0)} />
              <Stat
                label={`سود خالص (${unit})`}
            ...
            {data.stale && (
              <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>
                ⚠️ برخی نرخ‌های تبدیل کهنه‌تر از آستانه تعیین‌شده هستند؛ ارقام با آخرین قیمت موجود محاسبه شده‌اند.
              </div>
            )}
            {data.unpricedAssets.length > 0 && (
              <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>
                ⚠️ این دارایی‌ها در جمع کل لحاظ نشده‌اند چون نرخ تبدیل زنده‌ای نداشتند:{" "}
                {data.unpricedAssets.map((a) => a.symbol.slug ?? "—").join("، ")}
              </div>
            )}
                    ...
                    {data.assets.map((a) => (
                      <tr key={a.symbol.id}>
                        ...
                        <td className="mono">
                          {a.rate === null ? (
                            <Badge kind="red">
                              {UNPRICED_REASON[a.unpricedReason ?? ""] ?? "بدون نرخ"}
                            </Badge>
                          ) : (
                            <>
                              {fmtNum(a.rate, 6)}
                              {a.rateStale && (
                                <Badge kind="gold">کهنه</Badge>
                              )}
                            </>
                          )}
                        </td>
                        <td className="mono">{a.revenueInReference === null ? "—" : fmtNum(a.revenueInReference, 0)}</td>
                        <td className="mono">{a.costInReference === null ? "—" : fmtNum(a.costInReference, 0)}</td>
                        <td className="mono">{a.netInReference === null ? "—" : fmtNum(a.netInReference, 0)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <tbody>
                  {(holdings.data.assets ?? []).map((a: any) => (
                    <tr key={a.symbol.id}>
                      ...
                      <td className="mono">{a.rate === null ? "—" : fmtNum(a.rate, 6)}</td>
                      <td className="mono">{a.customerTotalInReference === null ? "—" : fmtNum(a.customerTotalInReference, 0)}</td>
                      <td className="mono">{a.systemBalanceInReference === null ? "—" : fmtNum(a.systemBalanceInReference, 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
