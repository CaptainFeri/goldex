...
export const bankAccountsApi = {
  list: async (
    ...
    unwrap<Paginated<AdminBankAccount>>(
      (await api.get("/admin/bank-accounts", { params })).data,
    ),

  get: async (id: string) =>
    unwrap<AdminBankAccount>(
      (await api.get(`/admin/bank-accounts/${id}`)).data,
    ),

  create: async (body: Partial<AdminBankAccount>) =>
    unwrap<AdminBankAccount>(
      (await api.post("/admin/bank-accounts", body)).data,
    ),

  update: async (id: string, body: Partial<AdminBankAccount>) =>
    unwrap<AdminBankAccount>(
      (await api.patch(`/admin/bank-accounts/${id}`, body)).data,
    ),

  /** Turn either direction on or off. Both may be true, both may be false. */
  setDirections: async (
    id: string,
    body: { useForDeposit: boolean; useForWithdraw: boolean },
  ) =>
    unwrap<AdminBankAccount>(
      (await api.patch(`/admin/bank-accounts/${id}/directions`, body)).data,
    ),
  ...
  setStatus: async (id: string, status: AdminBankAccount["status"]) =>
    unwrap<AdminBankAccount>(
      (await api.patch(`/admin/bank-accounts/${id}/status`, { status })).data,
    ),
};
...
export const p2pApi = {
  ...
  listEscalations: async (
    ...
    unwrap<Page<P2pEscalation>>(
      (await api.get("/admin/p2p/escalations", { params })).data,
    ),
    ...
    unwrap<P2pEscalation>(
      (
        await api.post(`/admin/p2p/escalations/${id}/resolve`, body, {
          ...
          headers: {
            "Idempotency-Key": `esc-${id}-${body.resolution}-${Date.now()}`,
          },
        })
      ).data,
    ),
  ...
  listMatches: async (
    ...
    unwrap<Page<P2pMatch>>(
      (await api.get("/admin/p2p/matches", { params })).data,
    ),
  ...
  listWithdrawals: async (
    ...
    unwrap<Page<P2pWithdrawRow>>(
      (await api.get("/admin/p2p/withdrawals", { params })).data,
    ),

  /** The whole case in one call: parts, live matches, receipts, escalation. */
  getWithdrawal: async (id: string) =>
    unwrap<P2pWithdrawDetail>(
      (await api.get(`/admin/p2p/withdrawals/${id}`)).data,
    ),
  ...
  escalateMatch: async (id: string, body: { reason: string; note?: string }) =>
    unwrap<P2pEscalation>(
      (await api.post(`/admin/p2p/matches/${id}/escalate`, body)).data,
    ),

  getSettings: async () =>
    unwrap<P2pSettings>((await api.get("/admin/p2p/settings")).data),
