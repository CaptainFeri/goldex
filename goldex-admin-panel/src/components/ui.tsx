...
export function Badge({
  ...
  const safe =
    ...
    !("$$typeof" in (children as any))
      ? ((children as any).slug ??
        (children as any).name ??
        (children as any).code ??
        "—")
      : children;
...
export function Loading({ label = "در حال بارگذاری…" }: { label?: string }) {
  return (
    <div className="center-state">
      <span className="spin" />{" "}
      <span style={{ marginInlineStart: 10 }}>{label}</span>
    </div>
  );
}
...
export function ErrorState({ message }: { message: string }) {
  return (
    <div className="center-state" style={{ color: "#f0857d" }}>
      ⚠ {message}
    </div>
  );
}
