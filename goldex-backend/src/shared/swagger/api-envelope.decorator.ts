 ...
 */
export const ApiEnvelopePrimitiveResponse = (
  type: Primitive,
  options: EnvelopeOptions = {}
) =>
  applyDecorators(
    ...
    ApiResponse({
      ...
      description: options.description,
      schema: envelopeSchema(
        options.isArray ? { type: "array", items: { type } } : { type }
      ),
    })
  );
...
export const ApiAdminErrorResponses = () =>
  applyDecorators(
    ...
    ApiResponse({ status: 403, description: "Authenticated, but lacking the required permission", type: ErrorEnvelopeDto }),
