...
function statusSheet(snapshot: WalletSnapshot): XLSX.WorkSheet {
  const rows: (string | number)[][] = [
    ...
    ['موجودی ریال (ریال)', snapshot.irrBalance],
    ['ارزش دارایی‌ها (نقد + بهای تمام‌شده طلا)', snapshot.equity],
    ['ذخیره نقدی (ریال)', snapshot.cashReserve],
    ['قدرت خرید (ریال)', snapshot.buyingPower],
    ['سود کل تحقق‌یافته (ریال)', snapshot.totalRealizedProfit],
