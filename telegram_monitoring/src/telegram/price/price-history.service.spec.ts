...
describe('PriceHistoryService arbitrage', () => {
  ...
  it('ignores opportunities below the minimum profit threshold (default 80,000)', () => {
    ...
    expect(service.detectArbitrage(sell)).toBeNull();
  });
  ...
  it('does not mix different sub-types or delivery types', () => {
    ...
    expect(service.detectArbitrage(shenaSell)).toBeNull();
  });
  ...
  it('does not pair legs observed too far apart in time', () => {
    record('73,500,000 🔴فروش⏳با حواله 1 تا', 1, 1000);
    // The خرید leg arrived 61s after the فروش — beyond the 60s leg-age window.
    const sell = record('73,600,000 🔵خرید⏳با حواله 1 تا', 2, 1061);
    expect(service.detectArbitrage(sell)).toBeNull();
  });

  it('pairs the current message with a recent opposite leg within 60s', () => {
    record('73,500,000 🔴فروش⏳با حواله 1 تا', 1, 1000);
  ...
  it('detects arbitrage when the current message is the فروش (we buy) side', () => {
    record('73,600,000 🔵خرید⏳با حواله 1 تا', 1, 1000); // we sell 73.6M
  ...
  it('attaches the buy/sell order buttons from the source snapshots', () => {
    ...
    const buySnap = service.record(buyParsed!, 1, 1000, {
      text: '1',
      data: 'grp|ord|111|1|1',
    });
    const sellParsed = parsePriceMessage('73,600,000 🔵خرید⏳با حواله 1 تا');
    expect(sellParsed).not.toBeNull();
    const sellSnap = service.record(sellParsed!, 2, 1010, {
      text: '1',
      data: 'grp|ord|222|2|1',
    });

    const opp = service.detectArbitrage(sellSnap);
    expect(opp).not.toBeNull();
